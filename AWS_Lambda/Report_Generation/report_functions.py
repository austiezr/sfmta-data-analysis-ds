...
def helper_count(expected_times, observed_times):
    ...
    for stop in expected_times.columns:
        for expected in expected_times[stop]:
            ...
            # or outbound definitions for a route.  That throws a key error here.
            # Example: stop 14148 on route 24
            # current solution is to ignore those stops with the try/except block
            try:
...
def create_simple_geojson(bunches, rid):
    """
    Returns a geojson object containing points for each bunch on the given route
...
def calculate_aggregate_report(all_reports):
    ...
    if all_reports[0]['route_id'] == "All":
        print("Warning: aggregate reports being calculated when aggregates already exists")
